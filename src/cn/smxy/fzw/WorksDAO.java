...
//		Connection conn = connection.getConnection();	//1、获取到了数据库userdb连接
		...
		callableStatement.setInt(1, user.getwid());
		callableStatement.setString(2, user.getwname());		
		callableStatement.setString(3, user.getWcontent());	
		callableStatement.setString(4, user.getP_time());	
		callableStatement.setString(5, user.getF_time());	
		//通过调用存储过程执行调用插入操作
